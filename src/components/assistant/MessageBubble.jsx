            ...
            {expanded && !statusConfig.spin && (
                <div className="mt-1.5 ml-3 pl-3 border-l-2 border-slate-200 space-y-2">
                    {toolCall.arguments_string && (
                        <div>
                            <div className="text-xs text-slate-500 mb-1">المعاملات:</div>
...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-7 w-7 rounded-lg bg-gradient-to-br from-emerald-100 to-emerald-200 flex items-center justify-center mt-0.5 flex-shrink-0">
                        ...
                        {isUser ? (
                            ...
                            <ReactMarkdown 
                                ...
                                components={{
                                    ...
                                    a: ({ children, ...props }) => (
                                        <a {...props} target="_blank" rel="noopener noreferrer" className="text-emerald-600 hover:text-emerald-700 underline">
                                            {children}
                                        </a>
                                    ),
                                    ...
                                    h1: ({ children }) => <h1 className="text-lg font-semibold my-2 text-emerald-800">{children}</h1>,
                                    h2: ({ children }) => <h2 className="text-base font-semibold my-2 text-emerald-700">{children}</h2>,
                                    h3: ({ children }) => <h3 className="text-sm font-semibold my-2 text-emerald-600">{children}</h3>,
                                    blockquote: ({ children }) => (
                                        <blockquote className="border-r-2 border-emerald-300 pr-3 my-2 text-slate-600 bg-emerald-50 py-2 rounded">
                                            {children}
                                        </blockquote>
                                    ),
                                }}
